...
class TestRepositoryScoringAlgorithms:
    ...
    def test_repository_ranking_by_score(self):
        """Test that repositories are ranked correctly by score."""
        analyzer = IntelligenceGuidedAnalyzer(Mock())
        ...
        ranked = analyzer._rank_repositories_by_score(repo_scores)
        
        # Should be in descending order
        expected_order = ["high-activity", "medium-activity", "low-activity", "inactive"]
        assert ranked == expected_order

    def test_repository_ranking_tie_handling(self):
        """Test behavior when repositories have identical scores."""
        analyzer = IntelligenceGuidedAnalyzer(Mock())
        
        # All repos have same score
        repo_scores = Counter({
            "repo-a": 5,
            "repo-b": 5,
            "repo-c": 5,
        ...
        # Should have all repos, order may vary but should be stable
        assert len(ranked) == 4
        assert set(ranked) == {"repo-a", "repo-b", "repo-c", "repo-d"}
